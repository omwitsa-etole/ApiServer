...
process.env.GOOGLE_APPLICATION_CREDENTIALS = path.join(__dirname, '../client_secret.json')
...
async function addWaterMark(pdfPaths, outputDir,controls,mode){
    ...
    for(var pdfPath of pdfPaths){
		...
		if(mode == 'text'){
			...
			const modifiedPdfBytes = await pdfDoc.save();
			console.log(pdfPath.server_filename,outputDir,pdfPath.filename)
			var outputFilePath = path.join(outputDir,pdfPath.filename);
			console.log(pdfPath.server_filename,outputDir,pdfPath.filename)
			fs.writeFileSync(outputFilePath, modifiedPdfBytes);
		}else{
			console.log("image=>",controls.image,pdfPath.server_filename,pdfPath.filename)
			var watermarkImagePath = path.join(__dirname, '../files/uploads/', controls.image)
			console.log("Resolved Path=>", watermarkImagePath);
			const watermarkImageBytes = fs.readFileSync(watermarkImagePath);
			const pages = pdfDoc.getPages();
			if (!pdfDoc || typeof pdfDoc.embedPng !== 'function') {
				throw new Error('PDFDocument is not initialized correctly.');
			}
			//console.log("Watermark Image Bytes Length: ", watermarkImageBytes.length);
			const fileExtension = path.extname(watermarkImagePath).toLowerCase();
			if (fileExtension === '.png') {
				const watermarkImage = await pdfDoc.embedPng(watermarkImageBytes);
			} else if (fileExtension === '.jpg' || fileExtension === '.jpeg') {
				const watermarkImage = await pdfDoc.embedJpg(watermarkImageBytes);
			} else {
			...
			console.log("image=>",controls.image)
			...
			pages.forEach(page => {
			  ...
			  const { x, y } =  getCoordinates(page, controls.text, fontSize,null,position,textWidth=scaledWidth);
			  console.log("image=>",controls.image,x,y)
