...
async function processTool(tool,data){
	switch(tool){
		...
		case 'pdfaudio':
			var new_req = new Requests({tool: 'pdfaudio',action: 'pdf_to_audio'})
			new_req.action = 'pdf_to_audio'
			var textContent = await Tool.extractTextFromPDF(data.files)
			var result = await Tool.convertTextToSpeech(textContent,path.join(__dirname,'../files/uploads/'+data.output_filename.replace(".pdf","")))
			if(result != null){
				new_req.success = true;
				await new_req.save()
				return {"sucess":true,output:result.split('uploads')[1].replace(/[\\]/g,""),tool: "audio"}
			}
			await new_req.save()
			return null
			break
